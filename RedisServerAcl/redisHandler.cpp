...
// // 이전 코드
//void handle_list_get(acl::redis& redis) {
//    std::string key;
//    int start, end;
//    acl::string value;
//
...
//
//    // std::vector<acl::string>* 포인터를 사용하여 lrange 호출
//    if (redis.lrange(acl_key, start, end, &list_values)) {
...
//void handle_list_create(acl::redis& redis) {
//    std::string key, value;
//    char action;
//
//    std::cout << "key를 입력하세요: ";
...
//        if (value == "q") break;
//
//        char* acl_value = new char[value.size() + 1];
//        std::strcpy(acl_value, value.c_str());
//        values.push_back(acl_value);
//    }
//
//    if (action == 'L' || action == 'l') {
//        if (redis.lpush(acl_key.c_str(), values) > 0) {
//            std::cout << "리스트에 값들이 성공적으로 왼쪽에 추가되었습니다." << std::endl;
//        }
//        else {
//            std::cerr << "리스트에 값을 추가하는 중 오류가 발생했습니다." << std::endl;
//        }
//    }
//    else if (action == 'R' || action == 'r') {
//        if (redis.rpush(acl_key.c_str(), values) > 0) {
//            std::cout << "리스트에 값들이 성공적으로 오른쪽에 추가되었습니다." << std::endl;
//        }
//        else {
//            std::cerr << "리스트에 값을 추가하는 중 오류가 발생했습니다." << std::endl;
//        }
...
//
//    acl::string acl_key(key.c_str());
//    std::vector<acl::string>* list_values = new std::vector<acl::string>();  // 포인터로 생성
//
//    redis.clear();
...
//    }
//
//    delete list_values;  // 포인터로 생성했으므로 해제
//}
